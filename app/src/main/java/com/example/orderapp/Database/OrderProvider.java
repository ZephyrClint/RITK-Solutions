...
public class OrderProvider extends ContentProvider {
    ...
    static {
        sUriMatcher.addURI(OrderContract.CONTENT_AUTHORITY, OrderContract.PATH1, ORDER);
    }
    ...
    private Uri getShippingInfo(Uri uri, ContentValues values) {

        String custName = values.getAsString(OrderContract.OrderEntry.SICUSTOMER_NAME);
        if(custName == null) {
            throw new IllegalArgumentException("Customer Name is Required");
        }

        String cardNum = values.getAsString(OrderContract.OrderEntry.SICARD_NUMBER);
        if(cardNum == null) {
            throw new IllegalArgumentException("Card Number is Required");
        }

        String cvv = values.getAsString(OrderContract.OrderEntry.SI_CVV);
        if(cvv == null) {
            throw new IllegalArgumentException("CVV is Required");
        }

        String expDate = values.getAsString(OrderContract.OrderEntry.SIEXP_DATE);
        if(expDate == null) {
            throw new IllegalArgumentException("CVV is Required");
        }

        String tpNum = values.getAsString(OrderContract.OrderEntry.SICONTACT_NUMBER);
        if(tpNum == null) {
            throw new IllegalArgumentException("Telphone is Required");
        }

        String dAddress = values.getAsString(OrderContract.OrderEntry.SIDEL_ADDRESS);
        if(dAddress == null) {
        ...
        long id = database.insert(OrderContract.OrderEntry.SITABLE_NAME, null, values);
